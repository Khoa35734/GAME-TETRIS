...
const Tetris: React.FC = () => {
  ...
  const drop = (): void => {
    ...
    if (!checkCollision(player, stage, { x: 0, y: 1 })) {
      ...
      // Logic game over sẽ được xử lý trong useEffect bên dưới
  ...
  // useEffect duy nhất để xử lý logic sau khi một khối được sinh ra
  useEffect(() => {
    // Chỉ chạy khi một khối mới được tạo ra (chưa va chạm)
    if (!player.collided) {
